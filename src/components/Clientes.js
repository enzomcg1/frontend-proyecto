...
const Clientes = () => {
    ...
    const [creditoAcumulado, setCreditoAcumulado] = useState(''); // Nuevo estado
    const [editarId, setEditarId] = useState(null);

    useEffect(() => {
        // Obtener lista de clientes cuando se monta el componente
    ...
    const agregarCliente = async (e) => {
        e.preventDefault();
        try {
            const nuevoCliente = { nombre, direccion, telefono, ci, creditoAcumulado }; // Incluye creditoAcumulado
            if (editarId) {
                // Actualizar cliente existente
                await axios.put(`http://localhost:4000/api/clientes/${editarId}`, nuevoCliente);
                setClientes(clientes.map(cliente => (cliente._id === editarId ? { ...cliente, ...nuevoCliente } : cliente)));
                setEditarId(null);
            } else {
                // Agregar nuevo cliente
                const res = await axios.post('http://localhost:4000/api/clientes', nuevoCliente);
            ...
            setTelefono('');
            setCI('');
            setCreditoAcumulado(''); // Limpiar creditoAcumulado
        } catch (error) {
    ...
    const iniciarEdicion = (cliente) => {
        ...
        setCI(cliente.ci);
        setCreditoAcumulado(cliente.creditoAcumulado); // Establece el crédito acumulado
            ...
            <form onSubmit={agregarCliente} className="mb-4">
                <div className="row">
                    ...
                    <div className="col-md-2">
                        <input
                            type="number"
                            placeholder="Crédito Acumulado"
                            value={creditoAcumulado}
                            onChange={(e) => setCreditoAcumulado(e.target.value)}
                            required
                            className="form-control mb-2"
                        />
                    </div>
                    ...
                    <div className="col-md-4 p-2" key={cliente._id}>
                        <div className="card">
                            <div className="card-header">
                                ...
                                <p>CI: {cliente.ci}</p>
                                <p>Crédito Acumulado: {cliente.creditoAcumulado}</p> {/* Mostrar crédito acumulado */}
                            </div>
